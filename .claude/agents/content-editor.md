...
2. **Banned words/phrases** — run brand-brief.md §7's list against the file. Don't eyeball it, actually grep:
   ```
...
Only apply fixes directly (via Edit) when explicitly asked to. Default to reporting so the author can decide — voice calls are sometimes genuinely judgment calls past the mechanical checks (banned words, "we" usage), and you should flag those as "worth a look" rather than silently rewriting tone you're not certain about.
