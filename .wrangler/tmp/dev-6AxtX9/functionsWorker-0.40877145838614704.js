...
// .wrangler/tmp/pages-rrAaWy/functionsWorker-0.40877145838614704.mjs
...
var MAX_BODY_LENGTH = 2e3;
var EMAIL_PATTERN = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
var json = /* @__PURE__ */ __name2((status, body) => new Response(JSON.stringify(body), {
  status,
  headers: { "content-type": "application/json" }
}), "json");
function parseCommentPayload(body) {
  if (!body || typeof body !== "object") return null;
  const { postSlug, authorName, authorEmail, body: message, turnstileToken, website } = body;
  if (typeof postSlug !== "string" || typeof authorName !== "string" || typeof authorEmail !== "string" || typeof message !== "string" || typeof turnstileToken !== "string" || typeof website !== "string") {
    return null;
  }
...
__name(parseCommentPayload, "parseCommentPayload");
__name2(parseCommentPayload, "parseCommentPayload");
...
var TURNSTILE_VERIFY_URL2 = "https://challenges.cloudflare.com/turnstile/v0/siteverify";
var MAX_FIELD_LENGTH = 2e3;
var EMAIL_PATTERN2 = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
var json2 = /* @__PURE__ */ __name2((status, body) => new Response(JSON.stringify(body), {
  status,
  headers: { "content-type": "application/json" }
}), "json");
function parseContactPayload(body) {
  ...
  if (typeof name !== "string" || typeof email !== "string" || typeof message !== "string" || typeof turnstileToken !== "string") {
    return null;
  }
  ...
  if (!EMAIL_PATTERN2.test(trimmedEmail)) return null;
...
async function verifyTurnstile2(token, secret, remoteIp) {
  const body = new URLSearchParams({ secret, response: token });
  if (remoteIp) body.set("remoteip", remoteIp);
  const response = await fetch(TURNSTILE_VERIFY_URL2, {
    method: "POST",
    headers: { "content-type": "application/x-www-form-urlencoded" },
    body
  });
...
async function signContactJwt(secret) {
...
__name(signContactJwt, "signContactJwt");
__name2(signContactJwt, "signContactJwt");
async function forwardToN8n(env, jwt, payload) {
...
__name(forwardToN8n, "forwardToN8n");
__name2(forwardToN8n, "forwardToN8n");
var onRequestPost2 = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    return json2(400, { ok: false, error: "Request body must be valid JSON." });
  }
  if (!payload) {
    return json2(400, { ok: false, error: "Missing or invalid name, email, message, or turnstileToken." });
  }
  try {
    const remoteIp = request.headers.get("CF-Connecting-IP");
    const isHuman = await verifyTurnstile2(payload.turnstileToken, env.TURNSTILE_SECRET_KEY, remoteIp);
    if (!isHuman) {
      return json2(400, { ok: false, error: "Human verification failed. Please try again." });
    }
  } catch (error) {
    console.error("Turnstile verification error:", error);
    return json2(502, { ok: false, error: "Unable to verify human validation at this time." });
  }
  let jwt;
  try {
    ...
    return json2(500, { ok: false, error: "Unable to authenticate the submission." });
  }
  try {
    await forwardToN8n(env, jwt, payload);
  } catch (error) {
    console.error("n8n webhook delivery error:", error);
    return json2(502, { ok: false, error: "Unable to deliver your submission at this time." });
  }
...
// .wrangler/tmp/bundle-yYsFIr/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-yYsFIr/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.40877145838614704.js.map
