...
- **PDF**: if reading directly can't render it (missing converter tools), fall back to `pdftotext -layout <file> <output>.txt` via terminal commands, then read the extracted text. Pull exact numbers from results tables, exact dataset citations, exact grant/funding info if present — don't round or paraphrase figures.
- **Live URL**: fetch headers (server, cache behavior, last-modified) and initial HTML/response body. This tells you what's actually deployed, which is not always what the repo's README claims. If the repo says a feature is live but the deployed response doesn't reflect it (a stubbed endpoint, a hardcoded placeholder), that mismatch is itself the most valuable finding — flag it prominently, don't bury it.
